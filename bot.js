...
bot.on('ready', function () {
	if (log1) console.log("UPX TipBot ready and loaded correctly! Hello, admin");
	bot.user.setActivity('!tipupx help');
});
function logBlockChainTransaction(incoming, authorId, paymentid, destination_wallet_address, blockheight, amount) {
	var dbo = db.db("upxTipBot");
	...
	if (isCallingBot(msg.content) == true) {
		...
		switch (command) {
			...
			case 'adminhelp':
				isAdmin(msg.author.id, function (result) {
					if (result == true) {
						msg.author.send("Hello! Welcome to the ADMIN HELP SECTION. \n To show user info, type \"!tipupx showuserinfo userid\" \n To add an admin, type \"!tipupx addadmin userid\" (only available for owner) \n To remove an admin, type \"!tipupx removeadmin userid\" (only available for owner) \n To disable tipping for a user, type \"!tipupx switchtipsend userid allow/disallow\" \n To disable receive of tips for a user, type \"!tipupx switchtipreceive userid allow/disallow\" \n To stop the bot from listening to commands, type \"!tipupx stoplistening\" \n To start bot listening to commands, type \"!tipupx startlistening\" (only available to bot owner) \n To display wallet info, type \"!tipupx walletinfo\" (only available to bot owner)");
					}
				});
				break;
			case 'help':
